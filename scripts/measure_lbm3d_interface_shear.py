"""Small 3D transverse shear-wave transfer gate, against uniform LBM.

A tangential velocity sine mode vanishes at both stationary walls. Density
and tangential velocity are compared on the common fine sampling grid.
This separates viscous momentum transport from the acoustic screen.
"""
...
def main(wavelength=192,wall_cells=12,output="reports/lbm3d_interface_shear_v1"):
    if wavelength not in (192, 96, 32, 16):
        raise ValueError("use a wall-compatible sine wavelength")
    h=96;nf=wall_cells;amplitude=1e-4;shape=(8,h+2,8)
    solid=np.zeros(shape,np.int32);solid[:,0,:]=1;solid[:,-1,:]=1
    def populations(s,y):
        rho=np.ones(s.shape)
        velocity=np.zeros((3,)+s.shape)
        velocity[0]=amplitude*np.sin(2*np.pi*y/wavelength)[None,:,None]
        return lb.quantize(lb.equilibrium(rho,velocity),s.bits)
    ...
    for done in range(50,2001,50):
        uniform.step(50);rho,u=lb.fields(uniform.numpy());truth=np.array([rho[:,1:-1,:].mean((0,2))-1,u[0,:,1:-1,:].mean((0,2))])
        ...
        for name,s in models.items():
            ...
            for block,ys,selection in [(s.fine[0],np.arange(nf)+.5,slice(1,-1)),(s.coarse,np.arange(nf+1,h-nf,2),slice(s.b,s.t+1)),(s.fine[1],np.arange(h-nf,h)+.5,slice(1,-1))]:
                ...
                positions.extend(ys);values.append(np.array([rho[:,selection,:].mean((0,2))-1,u[0,:,selection,:].mean((0,2))]))
    ...
    report={'scope':'small3D transverse shear interface diagnostic; not turbulent DNS','amplitude':amplitude,'wavelength_cells':wavelength,'wall_cells':nf,'active_cell_fraction':2*nf/h+(1-2*nf/h)/8,'steps':done,'seconds':time.perf_counter()-start,
            ...
            'source_sha256':{str(p):hashlib.sha256(p.read_bytes()).hexdigest() for p in [Path('src/kernel_engine/lbm/lbm3d_refinement.py'),Path('src/kernel_engine/lbm/lbm3d_refinement_gpu.py'),Path('scripts/measure_lbm3d_interface_shear.py')]}}
