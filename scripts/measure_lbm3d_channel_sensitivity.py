"""Finite-horizon coarsening sensitivity at the failed uniform DNS endpoint.
...
def observer(sim,mask,dns_report):
    ...
    def observe(q):
        ...
        for step in range(16,257,16):
            s.step(16)
            if step>128:wp.launch(accumulate,s.shape,[s.a,s.args[0],s.args[1],wp.float64(2**40),wp.int64(s.force_units),stats,s.failure],device='cuda:0')
        out=s.numpy();ledger=lb.ledger(out);wall=s.wall_impulse_numpy()
        balance=[ledger[k+1]-initial[k+1]+int(wall[k])-(256*dns.NX*dns.H*dns.NZ*s.force_units if k==0 else 0) for k in range(3)]
    ...
    for band in range(8):
        ...
        derivatives.append(responses[1]);report['probes'].append({'band':band,'distance_from_wall_cells':[band*4,(band+1)*4],
    ...
    report.update(weighted_jacobian=np.asarray(derivatives).T.tolist(),continuous_levels=levels.tolist(),
        ...
        candidate_wall_layers=8,candidate_active_cell_fraction=.25+.75/8,
        candidate_map_note='Two contiguous 8-cell wall blocks are a candidate only; ranking may reject this geometry.',
