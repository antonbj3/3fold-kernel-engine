...
def main(*,recursive=False,bulk_tau=None,device='cuda:0',output='reports/lbm3d_channel_laminar_v1/report.json'):
    ...
    for repeat in range(2):
        t=time.perf_counter();s=ch.ChannelSimulation(q,force_density=1e-5,tau=tau,cs=0,solid=mask,device=device,recursive=recursive,bulk_tau=bulk_tau)
    ...
    report={'preregistered':{'grid':[nx,h+2,nz],'fluid_height':h,'tau':tau,'bulk_tau':bulk_tau,'recursive':recursive,'device':device,'Cs':0,'steps':steps,'profile_L2_limit':.01,'peak_normalized_limit':.02},
