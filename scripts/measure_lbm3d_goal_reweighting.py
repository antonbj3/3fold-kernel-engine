...
def main(source=Path('reports/lbm3d_channel_sensitivity_v1/receipt/reports/lbm3d_channel_sensitivity_v1/report.json'),
         output=Path('reports/lbm3d_goal_reweighting_v1/report.json')):
    source=Path(source)
    ...
    for name,weights in masks.items():
        ...
        goals[name]=result
    report={'scope':'goal reweighting at one measured uniform DNS endpoint; no new PDE solve or certified mesh',
        'uniform_DNS_passed':r.get('uniform_DNS_passed',False),
