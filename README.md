...
**A fifth requirement, for loops.** A recurrence is read for its transition: diagonal, diagonal plus low rank, or associative means it can be computed in chunks, an outer sequential loop over chunks and an inner parallel loop inside, with the sequential result reproduced to round-off. On the public KernelBench set 7 of 17 stateful kernels change class from sequential to chunkable (cumulative sums, two Mamba-2 kernels); recurrent networks stay sequential. The rule is checked against its own failure: a kernel whose intra-chunk eigenvalues exceed one is refused, with the error growth measured. A numerical screen sits beside the structural rule: it measures the recurrence's transition gain and refuses chunking when the round-off grows past the output budget, as it does at gain 1.1 over 64 steps, where the error is 5.3 times the budget although the structure allows chunking.
...
The same accumulation now sits in eight of the engine's GPU reductions and in its automatic-differentiation adjoints, and each of those modules produces identical bytes across two runs. The whole family was run on four machines, an RTX 5070, an L4, an A10 and an H100: 120 pairwise comparisons, every one identical, and the list of operations that differ between the cards is empty. Each such module ships a certificate file with the input hash, the output hash per card and the driver and library versions, so a reader can re-check one on a rented card.

A kernel written and verified here can leave: the engine exports its CUDA C++ source with a small launch shim and builds it with nvcc into a program that has no Warp runtime. For the integer reduction and for the lattice-Boltzmann stream kernel the exported program gives identical bytes and 1.00 to 1.03 of the bandwidth of the Warp launch.

The same verified kernel was moved from CUDA to Vulkan on the same GPU and tested again: three repeated runs for identical bits, correctness against a 64-bit reference, a bit-for-bit comparison between the two APIs over four input sizes and four input patterns, and the bandwidth fraction. It kept its verification on all counts but one: numbers smaller than about 10⁻³⁸ are rounded to zero by the Vulkan driver and not by CUDA, so the bit-for-bit sweep fails on inputs that contain them. The bandwidth fractions are 90.0 % of the ceiling on CUDA and 94.1 % on Vulkan.
...
| Four cards, RTX 5070 / L4 / A10 / H100, 20 modules | 120 comparisons identical; differing-operation list empty |
| Warp → nvcc export, int64 reduction / LBM stream | identical bytes; bandwidth 1.03× / 1.00× of the Warp launch |
| Chunkable-recurrence rule on KernelBench | 7 of 17 stateful kernels chunkable; equivalence 3.3 × 10⁻⁶; eigenvalue > 1 refused |
| int64 fixed-point reduction time | 0.74× |
| CUDA → Vulkan port | 90.0 % / 94.1 % of the bandwidth ceiling; bit-for-bit sweep fails on subnormal inputs, flushed to zero on Vulkan |
| Chunkable stability screen | gain 1.1 over 64 steps: output error 7.4 × 10⁻⁵ against a budget of 1.4 × 10⁻⁵, chunking refused |
