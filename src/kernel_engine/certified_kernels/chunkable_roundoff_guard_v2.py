...
def main():
    ...
    np.savez_compressed(output / 'chunkable_roundoff_guard_v2.npz', **{f'{i}_{k}': a for i, leg in enumerate(arrays) for k, a in leg.items()})
    (output / 'chunkable_roundoff_guard_v2.json').write_text(json.dumps(dict(legs=reports, gates=gates), indent=2) + '\n')
