#!/usr/bin/env python3
"""
cell 530 — the SO-ARM100 CONTACT SIM-COST from the tropical-SDF backend (the compute value-chain capstone of my arc: w527 backend → w528 V6 curvature ladder →
w529 identifiability → w530 the COMPUTE COST, the #1 perception-compute-substrate deliverable = "how expensive is it to SIMULATE this contact"). The tropical-
SDF backend supplies the two contact quantities that set the explicit-integrator conditioning: PENETRATION δ (0-jet) and effective CURVATURE κ_eff (2-jet, w528).
...
below 2/ω_max and BLOW UP just above (the stability threshold VERIFIED, w504 pattern). PREREG: (a) the Hertz k_H rises with penetration δ (√δ) and with 1/√κ_eff
...
w527/528 (backend δ + κ_eff), w504 (stability-threshold verification), sim-conditioning-cert (κ/Δt/cost), heldout-cert (physics), cert-compression-oracle. External:
SO-ARM100 EE-tip geometry (r_cap=0.03 from the w528 backend) + Hertz elliptical contact (textbook). Machine-safe (OMP=4+nice, light).
"""
import os
...
def main():
    ...
    r_cap=0.03  # SO-ARM100 EE-tip cap radius (=1/κ_eff), from the w528 backend
    print("\n  BACKEND CONTACT: SO-ARM100 EE-tip sphere r_cap=%.3f m (κ_eff=1/r=%.1f from w528), m_eff=%.2f kg, E*=%.0e Pa, k_joint=%.0e N/m." % (r_cap,1/r_cap,M_EFF,E_STAR,K_JOINT))
    ...
    print("      Δt=0.9·Δt_crit → max|x|=%.2e (BOUNDED ✓) ; Δt=1.3·Δt_crit → max|x|=%s (DIVERGES ✓)" % (below,("%.1e"%above) if np.isfinite(above) else "inf"))
    ...
    if stability_ok and curvature_scales and scaling_ok:
        print("  ✓ DELIVERED (the COMPUTE VALUE-CHAIN CAPSTONE of my SO-ARM100 arc — backend w527 → V6 curvature w528 → identifiability w529 → SIM COST w530, the")
        ...
        print("    (w504 pattern). ★The cost SCALES with the geometry exactly as predicted: k_H∝√r_cap ⟹ steps∝r_cap^{1/4}, measured ratio %.3f vs analytic" % ratio_meas)
        ...
        print("    (different model, w529's caveat); E*/m/k_joint are representative scales — the SCALINGS (√δ, √r, the stability boundary) are the claim, not")
    ...
    print("  HYPOTHESIS+repro: OMP_NUM_THREADS=4 nice -n 15 python3 -u so_arm100_contact_sim_cost_from_tropical_sdf_backend_hertz_stiffness_sets_stable_dt_stability_verified.py")
