#!/usr/bin/env python3
"""
cell 496 — @D DECODE-WAVE @J package item: TROPICAL-SDF backend on SO-ARM100 (prereg ≥3× bytes + matched penetration). ★A robot arm's collision
...
(min-plus) structure (ties cert-composition-tropical-lattice + cert-as-task-conditioned-compression). So a tropical backend stores the SCENE as a
...
store N³ samples. ★PREREG: the tropical backend uses ≥3× FEWER bytes than the voxel resolution needed to MATCH its penetration-depth accuracy (the
...
match ⟹ ≫3× bytes. ★CONTROL: capsule SDF validated against the analytic point-to-segment distance − radius (a known geometry). @J package.
"""
import numpy as np
...
def main():
    ...
    print("\n  CONTROL (capsule SDF vs analytic point-to-segment−r): sdf=%s analytic=%s relerr=%.1e %s" % (np.round(sdf,4).tolist(),np.round(analytic,4).tolist(),np.abs(sdf-analytic).max(),"✓" if np.abs(sdf-analytic).max()<1e-12 else "✗"))
    ...
    if ok:
        print("  ✓ DELIVERED (@J DECODE-WAVE tropical-SDF item; prereg ≥3× bytes + matched penetration — EXCEEDED: the tropical DOMINATES on BOTH axes) —")
        ...
        print("    tropical SDF gives cheap EXACT penetration for the contact/κ cert (w488), and it composes across links (min-plus) with no grid. ⟹ use")
    ...
    print("  HYPOTHESIS+repro: python3 tropical_sdf_backend_min_of_capsules_beats_voxel_3x_bytes_matched_penetration_so_arm100.py")
