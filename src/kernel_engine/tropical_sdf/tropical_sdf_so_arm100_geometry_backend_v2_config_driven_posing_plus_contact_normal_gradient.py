#!/usr/bin/env python3
"""
cell 527 — J-2 DELIVERED (my D-assignment J-2 / EPOCH map coordinate "tropical-SDF SIMT-tax (J/U)", the #1 perception-compute-substrate region): the
TROPICAL-SDF SO-ARM100 GEOMETRY BACKEND v2 (decode-package). w496 proved tropical (min-of-capsules) SDF beats voxel ≥3× bytes-matched (dominance); w505
...
the demo can actually call MUST have and w496/w505 lacked: (1) CONFIG-DRIVEN POSING — the backend takes the robot STATE (joint angles) and poses the capsule
...
config-driven, from ~7 floats/capsule. PREREG (watertight, machine-checkable): (a) EIKONAL — the analytic min-plus normal matches the finite-difference
...
correctly as a joint angle sweeps (the arm moves through it → SDF crosses zero); (d) route note (w505): tropical wins at small K on GPU/fused; CPU-numpy is
dispatch-bound (voxel wins) — the backend carries the honest regime. Anchors: w496 (capsule core, dominance), w505 (SIMT-tax route), cert-composition-tropical
-lattice (min-plus = tropical union), kinematic-singularity (FK), scene-eyes. External anchor: SO-ARM100 capsule geometry (w496.so_arm100_capsules).
"""
import os
...
for _v in ("OMP_NUM_THREADS","OPENBLAS_NUM_THREADS","MKL_NUM_THREADS","NUMEXPR_NUM_THREADS"): os.environ.setdefault(_v,"4")  # machine-safety (cpu-pstate-secured)
...
def sdf(points, caps):
    """tropical (min-plus) union SDF — exact (w496)."""
...
def main():
    print("="*122); print("cell 527  J-2: TROPICAL-SDF SO-ARM100 GEOMETRY BACKEND v2 — config-driven posing + contact-normal gradient (min-plus subgradient)"); print("="*122)
    ...
    print("  (b) PENETRATION: interior point SDF=%.4f (<0 ✓=%s) ; exterior surface normal=%s (points OUT ✓=%s)" % (di[0],pen_ok,np.round(no[0],2),norm_out_ok))
    ...
    print("  (c) CONFIG-DRIVEN: query point SDF over joint-0 sweep ∈[%.3f,%.3f], crosses zero=%s → backend tracks the robot state ✓=%s" %
          (sweep.min(),sweep.max(),bool(sweep.min()<0<sweep.max()),config_ok))

    # (d) route (w505): the honest speed regime the backend carries
    print("  (d) ROUTE (w505 SIMT-tax): tropical is COMPUTE-bound O(K) → wins at small K on GPU/fused-kernel (SO-ARM100 K=%d deep in favorable regime);" % len(caps0))
    ...
    if eik_ok and pen_ok and norm_out_ok and config_ok:
        print("  ✓ DELIVERED (J-2: the TROPICAL-SDF SO-ARM100 GEOMETRY BACKEND v2, epoch #1 perception-compute-substrate region, decode-package) — a USABLE")
        print("    geometry backend the demo can call, adding the two features w496/w505 lacked: (1) CONFIG-DRIVEN POSING (serial FK: joint angles → posed")
        ...
        print("    is the right BACKEND (not merely cheaper): the min-plus union structure yields the normal at no extra cost. Carries the w505 route (tropical")
        print("    on GPU/fused at small K; voxel on CPU) so the consumer picks the right kernel. This packages w496 (dominance) + w505 (SIMT-tax) into the")
        ...
        print("    ★HONEST SCOPE: FK uses REPRESENTATIVE joint axes (roty, matching w496's SO-ARM100-representative capsule model) — a production backend reads")
    ...
    print("  HYPOTHESIS+repro: OMP_NUM_THREADS=4 nice -n 15 python3 -u tropical_sdf_so_arm100_geometry_backend_v2_config_driven_posing_plus_contact_normal_gradient.py")
