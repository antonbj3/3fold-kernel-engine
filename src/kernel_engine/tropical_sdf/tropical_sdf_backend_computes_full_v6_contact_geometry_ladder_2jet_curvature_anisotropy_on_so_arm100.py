#!/usr/bin/env python3
"""
cell 528 — the TROPICAL-SDF BACKEND computes the FULL V6 CONTACT-GEOMETRY OBSERVABLE LADDER on SO-ARM100 (unifies my w195/196 V6-curvature theory with my
w527 robot backend; graph-anchored in the ACTIVE V6 node = contact-floor/geometry, ↑↑). w195/196 proved (on generic Hertz surfaces) that the 2-JET of the
...
is isotropic → blind to κ1≠κ2). w527 built the SO-ARM100 tropical-SDF backend with the 0-jet (penetration) + 1-jet (contact normal). ★This composes them: the
...
ground truth. PREREG (watertight, machine-checkable vs analytic capsule curvature): (a) the 2-jet Hessian's two non-normal eigenvalues = the principal
...
CAP: {κ1,κ2}≈{1/r,1/r} → ISOTROPIC (|κ1−κ2|≈0); (d) the ladder RANK climbs force→+mean→+spectrum (w196 hierarchy) — the backend supplies all rungs.
Anchors: w195/196 (2-jet V6 curvature/anisotropy), w527 (SO-ARM100 backend 0+1 jet), V6 node (5-DOF ladder), S2 σ_min-crown (curvature raises observable rank),
cert-composition-tropical-lattice. External anchor: SO-ARM100 capsule geometry (analytic capsule curvature = the ground truth).
"""
import os
...
w527=_load("w527", _here("tropical_sdf_so_arm100_geometry_backend_v2_config_driven_posing_plus_contact_normal_gradient.py"))
...
def main():
    print("="*126); print("cell 528  TROPICAL-SDF BACKEND computes the FULL V6 CONTACT-GEOMETRY LADDER on SO-ARM100 — 2-jet Hessian curvature + anisotropy (w195/196 ⊗ w527)"); print("="*126)
    ...
    print("\n  (d) V6 OBSERVABLE-RANK LADDER (w196 hierarchy) — what each rung of the backend's 2-jet resolves:")
    ...
    print("      +2-jet MEAN curvature (Hessian trace) → +1 DOF (mean curvature) but force sees only κ_eff=√(κ1κ2) → still BLIND to anisotropy (w196)")
    ...
    if cyl_ok and cap_ok and aniso_separates:
        print("  ✓ DELIVERED (the tropical-SDF SO-ARM100 backend computes the FULL V6 CONTACT-GEOMETRY OBSERVABLE LADDER — unifies my w195/196 2-jet curvature")
        print("    theory with my w527 robot backend, graph-anchored in the active V6 node) — from ONE geometry backend the robot's contact gets: 0-jet")
        print("    PENETRATION, 1-jet NORMAL (w527), and now the 2-jet HESSIAN SPECTRUM → MEAN CURVATURE + ANISOTROPY. Validated against ANALYTIC capsule")
        ...
        print("    the load-bearing V6 result made concrete: w196 showed FORCE is isotropic (sees only κ_eff=√(κ1κ2)) → BLIND to anisotropy; only the full")
        ...
        print("    raises the V6/S2-σ_min observable rank on a real robot: force→+mean→+anisotropy climbs the w196 hierarchy from the SAME backend. ⟹ the")
    ...
    print("  HYPOTHESIS+repro: OMP_NUM_THREADS=4 nice -n 15 python3 -u tropical_sdf_backend_computes_full_v6_contact_geometry_ladder_2jet_curvature_anisotropy_on_so_arm100.py")
