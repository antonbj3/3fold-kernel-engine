#!/usr/bin/env python3
"""
cell 540 — SWEPT-VOLUME / CONTINUOUS COLLISION DETECTION (CCD) extension of my tropical-SDF SO-ARM100 backend (w527, which packages w496's
min-of-capsules union SDF + config-driven FK posing). ★The geometry insight (min-plus/tropical, the SAME lattice as w496/w527): the SDF of a union
...
PREREG (watertight, machine-checkable against w527's own posed_capsules/capsule_sdf, no new geometry primitive introduced):
...
Anchors: w496 (capsule union = tropical/min-plus SDF, exact), w527 (config-driven posed_capsules/sdf, contact-normal subgradient),
cert-composition-tropical-lattice (min-plus = tropical union, composes over ANY index — here TIME joins CAPSULE). External anchor: capsule_sdf
is the analytic point-to-segment-minus-radius distance (w496-validated to 1e-12); this file introduces NO new primitive, only a new min-axis (time).
"""
import os
...
def swept_sdf(obstacle_points, q0, q1, T, caps_fn=posed_capsules):
    ...
    S = Sk.min(axis=1)                                  # (T,M) tropical union SDF per time-sample (== w527.sdf at each posed config)
...
def main():
    ...
    # ---------------- shared collision scenario: joint-0 sweep through a fixed obstacle point (w527(c) confirmed this SDF dips negative) ----------------
    ...
    print("      endpoint SDF: q0-clearance=min(sdf(P,caps(q0)))=%.4f ; q1-clearance=min(sdf(P,caps(q1)))=%.4f  (both >0.05 ✓=%s)" %
          (sdf_q0.min(), sdf_q1.min(), endpoints_clear))
    print("      swept  SDF (T=41): min over all P & t = %.4f  (<-0.01 ✓=%s)  -> point idx=%d (p_hit itself=%s)" %
    ...
    print("\n  (c) MIN-PLUS WITNESS (first-contact / closest-approach): t*=%.4f (interior, 0<t*<1 ✓=%s), capsule*=%d" % (t_star, interior_ok, k_star))
    print("      cross-check: independently recomputed capsule_sdf(P,cap_k*)@config(t*) = %.6f  vs swept_sdf's own d_min = %.6f  (match ✓=%s)" %
    ...
    print("      shrink T=3->101: %.1fx (need >=10x ✓=%s) ; non-increasing steps=%d/6 (need >=5/6 ✓=%s) ; ref converged ✓=%s" %
    ...
    if all_ok:
        ...
        print("    -> swept-volume CCD is the SAME tropical lattice as w496/w527 with one more min-axis (TIME); no new primitive, no new failure mode.")
    else:
        print("  FAIL/INSPECT — a=%s b=%s c=%s d=%s (see per-check numbers above)." % (a_ok, b_ok, c_ok, d_ok))
    print("\n  HYPOTHESIS+repro: OMP_NUM_THREADS=4 nice -n 15 python3 -u tropical_sdf_swept_volume_continuous_collision_detection_min_over_trajectory.py")
