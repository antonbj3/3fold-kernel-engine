"""Experimental mixed collision: FP32 moment transforms, FP64 fields/reconstruction.

Derived source SHA256: 4befcbd9dd31de11fe5f54a3af4f9734d7155a0f8c08c1a15cae34d4f0f91a94
Not admitted for turbulent DNS without precision gates.
"""
import warp as wp
...
V19f=wp.types.vector(length=19,dtype=wp.float32)
Mat19f=wp.types.matrix(shape=(19,19),dtype=wp.float32)
FIXED_M=wp.constant(Mat19f(M_HERMITE))
FIXED_MI=wp.constant(Mat19f(MI_HERMITE))

@wp.func
def transform32(matrix:Mat19f,values:V19):
    low=V19f()
    for i in range(19):low[i]=wp.float32(values[i])
    product=matrix*low
    high=V19()
    for i in range(19):high[i]=wp.float64(product[i])
    return high

@wp.kernel
def mixed_collide_stream(a: wp.array4d(dtype=wp.int64), b: wp.array4d(dtype=wp.int64),
                ...
                if h >= 4:
                    ...
                    if h < 10:
                        ...
                        if h == 4 and bulk_rate > wp.float64(0.0):
                            rate = bulk_rate
                relaxed[h] = rate*dm[h]
            post = g-transform32(FIXED_MI,relaxed)
        if recursive != 0:
...
class MixedChannelSimulation(ChannelSimulation):
    ...
    def step(self,steps=1):
        ...
        for _ in range(steps):
            wp.launch(mixed_collide_stream,self.shape,[self.a,self.b,*self.specialized_args],device=self.device)
