"""Experimental regularized32 reconstruction from retained moments.
Real-arithmetic equivalent to full relaxation; floating parity must be gated.
"""
import warp as wp
...
@wp.kernel
def regularized32_collide_stream(a: wp.array4d(dtype=wp.int64), b: wp.array4d(dtype=wp.int64),
    ...
    for q in range(19):
        fq = a[q,i,j,k]
        qout[q] = fq
        g[q] = wp.float32(wp.float64(fq)/scale64-w[q])
        mass += fq
    ...
    if solid[i,j,k] != 0:
        for q in range(19):
            b[q,i,j,k] = a[q,i,j,k]
        return
    rho_delta=wp.float32((wp.float64(mass)-scale64)/scale64)
    rho = wp.float32(1.0)+rho_delta
    ...
    if valid:
        ...
        for q in range(19):
            ...
            eq[q] = wp.float32(w[q])*(rho_delta+rho*(wp.float32(3.0)*cu+wp.float32(4.5)*cu*cu-wp.float32(1.5)*usq))
            if recursive != 0:
                ...
                if h >= 4:
                    ...
                    if h < 10:
                        ...
                        if h == 4 and bulk_rate > wp.float32(0.0):
                            rate = bulk_rate
                relaxed[h] = (wp.float32(1.0)-rate)*dm[h]
            post = eq+FIXED_MI*relaxed
        if recursive != 0:
        ...
        if valid:
            ...
            for q in range(19):
                fq = wp.int64(wp.round((wp.float64(post[q])+w[q])*scale64))
                qout[q] = fq
                outmass += fq
...
class Regularized32ChannelSimulation(ChannelSimulation):
    ...
    def step(self,steps=1):
        ...
        for _ in range(steps):
            wp.launch(regularized32_collide_stream,self.shape,[self.a,self.b,*self.specialized_args],device=self.device)
