"""Experimental split FP64 collision/streaming with one extra population buffer.
Arithmetic and exact integer wall accounting retain the fused order.
"""
import warp as wp
...
@wp.kernel
def collide_to_post(a: wp.array4d(dtype=wp.int64), b: wp.array4d(dtype=wp.int64),
    ...
    for q in range(19):b[q,i,j,k]=qout[q]

@wp.kernel
def stream_post(a:wp.array4d(dtype=wp.int64),b:wp.array4d(dtype=wp.int64),solid:wp.array3d(dtype=wp.int32),c:wp.array2d(dtype=wp.int32),opp:wp.array(dtype=wp.int32),nx:int,ny:int,nz:int,failure:wp.array(dtype=wp.int32),wall_impulse:wp.array(dtype=wp.int64)):
    i,j,k=wp.tid()
    qout=I19()
    for q in range(19):qout[q]=a[q,i,j,k]
    if solid[i,j,k]!=0:
        for q in range(19):b[q,i,j,k]=qout[q]
        return
    for q in range(19):
...
class SplitChannelSimulation(ChannelSimulation):
    def __init__(self,*args,**kwargs):
        super().__init__(*args,**kwargs)
        self.specialized_args=self.args[:4]+self.args[6:]
        self.post=wp.empty_like(self.a)
    def step(self,steps=1):
        if not isinstance(steps,int) or steps<0:raise ValueError('invalid steps')
        for _ in range(steps):
            wp.launch(collide_to_post,self.shape,[self.a,self.post,*self.specialized_args],device=self.device)
            wp.launch(stream_post,self.shape,[self.post,self.b,self.args[0],self.args[1],self.args[3],*self.shape,self.failure,self.wall_impulse],device=self.device)
