"""Experimental regularized64 reconstruction from retained moments.
...
@wp.kernel
def regularized64_collide_stream(a: wp.array4d(dtype=wp.int64), b: wp.array4d(dtype=wp.int64),
                ...
                if h >= 4:
                    ...
                    if h < 10:
                        ...
                        if h == 4 and bulk_rate > wp.float64(0.0):
                            rate = bulk_rate
                relaxed[h] = (wp.float64(1.0)-rate)*dm[h]
...
class Regularized64ChannelSimulation(ChannelSimulation):
    ...
    def step(self,steps=1):
        ...
        for _ in range(steps):
            wp.launch(regularized64_collide_stream,self.shape,[self.a,self.b,*self.specialized_args],device=self.device)
