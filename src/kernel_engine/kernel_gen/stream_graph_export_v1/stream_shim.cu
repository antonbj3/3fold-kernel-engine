// Captured dispatch through the existing C ABI and frozen generated kernel.
#include "builtin.h"
...
extern "C" int stream_export_run(const float *source, const int32_t *mask,
    ...
    try {
        ...
        auto launch=[&]() {
            stream_3bfa5cbb_cuda_kernel_forward<<<(cells+255)/256,256,0,s.stream>>>(bounds,a,b,m,x,y,o,n,n);
            check(cudaGetLastError());
        };
        check(cudaStreamBeginCapture(s.stream,cudaStreamCaptureModeGlobal));
        for (int i=0;i<30;++i) launch();
        check(cudaStreamEndCapture(s.stream,&s.graph));
        check(cudaGraphInstantiate(&s.executable,s.graph,0));
        check(cudaGraphLaunch(s.executable,s.stream));
        check(cudaStreamSynchronize(s.stream)); check(cudaMemcpy(output,s.out,bytes,cudaMemcpyDeviceToHost));
        std::vector<float> repeat(9*cells);
        check(cudaGraphLaunch(s.executable,s.stream));
        check(cudaStreamSynchronize(s.stream)); check(cudaMemcpy(repeat.data(),s.out,bytes,cudaMemcpyDeviceToHost));
        ...
        check(cudaEventRecord(s.begin,s.stream));
        check(cudaGraphLaunch(s.executable,s.stream));
        check(cudaEventRecord(s.end,s.stream)); check(cudaDeviceSynchronize());
