...
int main(int argc, char** argv) {
    ...
    try {
        ...
        auto phase = [&](const char* name) {
            auto now = Clock::now();
            phases.emplace_back(name, std::chrono::duration<double,std::milli>(now-mark).count());
            mark=now;
        };
        ...
        phase("sbt_ray_upload");
        constexpr int repetitions=64;
        std::vector<int> result(rays), first_result;
        double query_sum=0,query_min=1e30,query_max=0;
        for (int iteration=0;iteration<repetitions;++iteration) {
            const auto begin=Clock::now();
            check(cudaMemcpy(reinterpret_cast<void*>(ray_data), origins.data(), origins.size()*sizeof(float3), cudaMemcpyHostToDevice));
            ...
            const double duration=std::chrono::duration<double,std::milli>(Clock::now()-begin).count();
            query_sum+=duration; query_min=std::min(query_min,duration); query_max=std::max(query_max,duration);
            if (iteration==0) first_result=result;
            else if (result!=first_result) throw std::runtime_error("batch output mismatch");
        }
        phase("batch_upload_launch_readback");
        phases.emplace_back("query_mean",query_sum/repetitions);
        phases.emplace_back("query_min",query_min);
        phases.emplace_back("query_max",query_max);
        std::ofstream out(argv[3], std::ios::binary);
        out.write(reinterpret_cast<char*>(result.data()), result.size()*sizeof(int));
        if (!out) throw std::runtime_error("output write failed");
        phase("readback_write");
        ...
        for (size_t i=0; i<phases.size(); ++i) {
            if (i) std::cout << ",";
            std::cout << "\"" << phases[i].first << "\":" << phases[i].second;
        }
        std::cout << "}\n";
        return 0;
